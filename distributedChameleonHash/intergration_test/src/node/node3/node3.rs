...
{
    let log_path = String::from(env::current_dir().unwrap().as_path().to_str().unwrap())+"/src/node/node3/config/config_file/log4rs.yaml";
    log4rs::init_file(log_path, Default::default()).unwrap();
    
    let gs_tbk_config_path  = String::from(std::env::current_dir().unwrap().as_path().to_str().unwrap())+"/src/node/node3/config/config_file/node_config.json";
    ...
    info!("node3 is listening on {}",node.address);
                ...
                GSTBKMsg::GSTBKMsgP(gstbk_proxy_msg) => 
                {
                    match gstbk_proxy_msg
                    {
                        message::proxy::common_msg::GSTBKMsg::SetupMsg(setup_msg) => 
                        {
                            match setup_msg 
                            {
                                ...
                                message::proxy::common_msg::SetupMsg::ProxySetupPhaseFinishFlag(_msg) => 
                                {
                                    ...
                                    match p2p(keygen_phase_one_msg_str, (*locked_node.proxy_address).to_string()).await 
                                    {
                                        Ok(_) => {}
                                        Err(e) => 
                                        {
                        ...
                        message::proxy::common_msg::GSTBKMsg::KeyGenMsg(keygen_msg) => 
                        {
                            match keygen_msg  
                            {
                                message::proxy::common_msg::KeyGenMsg::ProxyToNodeGetSTotal(msg) => {
                                    info!("Get ProxySetupPhaseFinishFlag");
                                    let mut locked_node = node.lock().await;
                                    locked_node.keygen_phase_two(msg);
                                }
                            }
                        }
                    }
